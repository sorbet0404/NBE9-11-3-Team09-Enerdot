...
- 초기 프로젝트 세팅 및 Reservation 도메인 전담 — 예약 생성·조회·취소 API 설계 및 구현
- 전역 예외 처리 및 공통 응답 포맷 구조 수립으로 팀 개발 표준 정의
- Swagger(OpenAPI) 기반 API 명세서 작성으로 프론트엔드 협업 효율화
- Next.js 15 + TypeScript 기반 프론트엔드 API 연동 및 주요 UI 화면 구현
- 외부 의존성 없이 DB 비관적 락으로 동시성 제어를 시도했으나, 팀원 제안으로 CAS 방식 전환 후 k6 부하 테스트로 검증
